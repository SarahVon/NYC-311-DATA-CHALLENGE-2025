  ...
  tabItems(
    ...
    tabItem(tabName = "About",
            fluidPage(
              ...
              p("The primary purpose of this dashboard is to explore and analyze the NYC 311 Service Request data and to offer insights into complaint patterns, geographic distribution, and response times across different boroughs. The dashboard is designed to guide users in understanding public service efficiency and identifying potential areas for improvement."),
              h3("Statistical Analysis Documentation"),
              p("This dashboard dynamically filters and visualizes service request data to provide insights into submission trends, response times, and public service efficiency across New York boroughs. The general overview section includes a heatmap which categorizes 191 unique submission types into seven distinct categories in order to highlight major trends. Users can also interact with a Leaflet map displaying the five boroughs while also looking at a plot of the top ten complaints for each borough. The average response time section includes an interactive bar chart comparing response efficiency by complaint type and a time series plot tracking response time trends. Response time was calculated as the difference between the creation date of each submission and the close date. This allowed for a clear comparison of service efficiency across submission types and boroughs. This section also includes a requests-over-time plot which uses an average response time trendline for each borough. These plots will allow users explore how submission response time changes throughout the year. Users can filter data using dropdown menus and date sliders, allowing for an in-depth exploration of submission trends. Lastly, this dashboard includes an interactive plot showcasing the top ten parks with the most submissions, along with the borough each park lies in."),
              h3("Limitations"),
              p("There are several limitations to consider. Data quality issues, such as missing or incomplete records, may impact the accuracy of the analysis. Additionally, certain boroughs may have more complaints reported due to higher awareness or easier access to reporting channels. Response times may also vary depending on the nature of the complaint, and external factors like seasonal trends or public events could influence complaint frequencies and response times."),
              h3("Challenge and Dashboard Goals"),
              p("The challenge addressed by this dashboard is to transform the NYC 311 service request dataset into an interactive tool that not only visualizes the data but also provides actionable insights for users. The goals include analyzing response efficiency, service trends, and borough-level variations."),
              h3("Links & Documentation"),
              p("This project is based on the NYC 311 service request dataset, which contains information on complaints submitted by residents regarding various city issues. This challenge uses data from 2024"),
